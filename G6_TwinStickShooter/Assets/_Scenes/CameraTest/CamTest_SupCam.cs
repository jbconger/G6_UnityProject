...
public class CamTest_SupCam : MonoBehaviour
{
    public CamTest_LevelFocus CamTest_LevelFocus;
    ...
    void Start()
    {
        Players.Add(CamTest_LevelFocus.gameObject);
    }
    ...
    private void CalculateCameraLocations()
    {
        ...
        for (int i = 0; i < Players.Count; i++)
        {
            ...
            if (!CamTest_LevelFocus.FocusBounds.Contains(playerPosition))
            {
                float playerX = Mathf.Clamp(playerPosition.x, CamTest_LevelFocus.FocusBounds.min.x, CamTest_LevelFocus.FocusBounds.max.x);
                float playerY = Mathf.Clamp(playerPosition.y, CamTest_LevelFocus.FocusBounds.min.y, CamTest_LevelFocus.FocusBounds.max.y);
                float playerZ = Mathf.Clamp(playerPosition.z, CamTest_LevelFocus.FocusBounds.min.z, CamTest_LevelFocus.FocusBounds.max.z);
        ...
        float lerpPercent = Mathf.InverseLerp(0, (CamTest_LevelFocus.HalfXBounds + CamTest_LevelFocus.HalfYBounds) / 2, extents);
